...
function processSum(numberList, callback) {
  return callback(numberList.reduce(function(acc, val) { return acc + val; }, 0)
  );
}
...
const doesListIncludeItem = list.includes(item);
return callback(doesListIncludeItem);
}
...
function getFullNames(runners) {
  const fullNames = [];
  runners.forEach(function(singleRunner) {
    const formattedName = `${singleRunner.last_name}, ${singleRunner.first_name}`;
    fullNames.push(formattedName);
  });
...
function firstNamesAllCaps(runners) {
  return runners.map(function(singleRunner) {
    return singleRunner.first_name.toUpperCase();
  });
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  return runners.filter(function(singleRunner){
    return singleRunner.shirt_size === tShirtSize;
  });
}
...
function tallyUpDonations(runners) {
  return runners.reduce(function(total, singleRunner) {
    return total + singleRunner.donation;
  }, 0);
}
